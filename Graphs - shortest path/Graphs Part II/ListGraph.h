...
public:
    ...
    // Construct list graph from matrix graph
    ListGraph(MatrixGraph* matrix_graph) {
        this->size = matrix_graph->getSize();
        nodes.resize(this->size);
        my_edges = matrix_graph->get_whole_edges();
        for(int i = 0; i < my_edges.getSize(); ++i) {
            addEdge(my_edges.getEdgeByIndex(i).src, my_edges.getEdgeByIndex(i).dest);
        }
    }
    ...
    void print_BFS(int root, int visited[] = NULL)
    {
    ...
    void distances_BFS(int root, int my_distances[] = NULL)
    {
        // Create queue with the root
        std::queue<int> my_queue;
        my_queue.push(root);
